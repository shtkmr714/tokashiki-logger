...
# 欠航時の那覇ホテル記事（慶良間＝泊港発。座間味・渡嘉敷 共通の集客導線）
NAHA_HOTELS_URL = "https://okinawaislandguide.github.io/articles/ferry-cancelled-naha-hotels.html"
NAHA_CTA_THRESHOLD = 50   # 明日・明後日の最大欠航%がこれ以上でCTAを追記


def _naha_hotels_cta(forecast):
    """短期の欠航リスクが高い日だけ、那覇の宿確保への導線を返す（無ければ空文字）。"""
    try:
        vals = [v for d in forecast.get("short_term", [])[:2]
                for v in (d.get("hs_pct") or 0, d.get("fe_pct") or 0)]
        if not vals or max(vals) < NAHA_CTA_THRESHOLD:
            return ""
    except Exception:
        return ""
    return (
        "\n\n🏨 欠航が心配な日は、那覇の宿も早めに確保を。"
        "泊港・国際通り周辺の当日予約OKなおすすめ宿はプロフィールのリンクから。"
        "\n🏨 Worried about a cancellation? Line up a Naha hotel early — "
        "our picks near Tomari Port & Kokusai-dori (link in bio)."
        "\n" + NAHA_HOTELS_URL
    )
...
def _build_caption(forecast, now, suspensions=None):
    ...
    return ig_caption + _naha_hotels_cta(forecast)
